...
function reverseString(str) {
    ...
    return strCopy.pop() + reverseString(strCopy.join(''));
}
...
function fileFinder(directories, targetFile) {
    for (let key in directories) { 
        ...
        //     return fileFinder(value, targetFile)
        // })
    }
...
function pathFinder(directories, targetFile) {
    ...
    for (let key in directories) {
        root = ''
        // path += key
        // if (key === targetFile || fileFinder(directories[key], targetFile)) return path += '/' + targetFile
        if (key === targetFile) {
